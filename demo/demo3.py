...
class ZLTabWidget(QTabWidget):
    def __init__(self, orangeMainWin):
        ...
        webMainWin = ZLWebMainWindow()
        for title, url in ZLConfig.configs.items():
            index = ZLConfig.get_index(title)
            if title == '人工智能':
                index1 = self.addTab(orangeMainWin, f'tab_{index}')
            else:
                index1 = self.addTab(webMainWin.set_config(title=title, url=url), f'tab_{index}')
            print(index1)
...
if __name__ == '__main__':
    ...
    demo = ZLTabWidget(ZLWebMainWindow().set_config('人工智能', 'https://yiyan.baidu.com/'))
    demo.show()
    sys.exit(app.exec_())
